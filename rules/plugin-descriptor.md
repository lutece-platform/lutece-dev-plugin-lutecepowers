...
The feature icon is read from `<icon-url>` (`plugin-digester-rules.xml`, pattern `admin-features/admin-feature`) and rendered as a CSS class by `adminHeader.ftl`: a Tabler class `ti ti-<name>`, never an image path. `<feature-icon-url>` is not read.
...
| Empty feature title, WARN `Error localizing key` in the log | Prefix missing in feature-title/description | Use `pluginName.key` format |
