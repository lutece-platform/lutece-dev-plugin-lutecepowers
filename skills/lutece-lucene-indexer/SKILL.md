...
The class that turns one business object into a Lucene `Document` and drives the `IndexWriter`: the full
model, with its imports, its field mapping and its incremental branch, is in
[reference/indexer-implementation.md](reference/indexer-implementation.md).
