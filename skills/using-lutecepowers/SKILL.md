...
| `lutece-e2e` | Gives any Lutece 8 core, plugin, module or site an `e2e/` bench that runs with one command: isolated Docker stack, seeded volume, inventory of every back-office screen and action, Playwright suites, perf capture and a report. `run.sh compare` also proves a migration's upgrade path on a taken-over database. |
