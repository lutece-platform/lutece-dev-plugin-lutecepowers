...
rm -f "$X/a.html"; printf '%s\n' "<#macro picker cuid=''>" "<script>const c = { cuid: '\${cuid}' };</script>" "</#macro>" > "$X/picker.ftl"
expect "TD63: a macro file (.ftl) is scanned, a customer id counts as text" 1 "$(td TD63)"
rm -f "$X/picker.ftl"
...
printf '%s\n' '<a href="x?q=${q?url}">x</a><#-- ${c?url} -->' > "$X/a.html"
expect "TD66: ?url without a charset" 1 "$(td TD66)"
printf '%s\n' "<a href=\"x?q=\${q?url('UTF-8')}\">x</a>" > "$X/a.html"
expect "TD66: ?url with its charset" 0 "$(td TD66)"
printf '%s\n' '<#setting url_escaping_charset="UTF-8">' '<a href="x?q=${q?url}">x</a>' > "$X/a.html"
expect "TD66: charset declared by the template" 0 "$(td TD66)"
printf '%s\n' "<@cInput type='hidden' name='to_date' value='' />" "<@cInput name='to_date' id='to_date' value='' />" "<script>flatpickr( document.getElementById( 'to_date' ) );</script>" > "$X/a.html"
